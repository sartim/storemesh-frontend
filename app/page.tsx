...
export default function Home() {
  ...
  return <main className={isAdmin(session) && activeView === "admin" ? "admin-shell" : "shell"}>
    {activeView === "shop" && <section className="card cart-summary"><span className="eyebrow">SAVED CART · {cart.lines?.reduce((sum, line) => sum + (line.quantity ?? 0), 0) ?? 0} ITEMS</span><p className="muted">Your cart is saved to your account and follows you across devices.</p>{cartError && <p className="error">{cartError}</p>}<button type="button" onClick={() => selectedProduct && void addToCart(selectedProduct)} disabled={!selectedProduct}>Add selected product</button>{(cart.lines?.length ?? 0) > 0 && <button type="button" className="ghost" onClick={() => void clearCart()}>Clear cart</button>}</section>}
