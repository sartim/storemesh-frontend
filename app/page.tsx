...
function productImage(product: Product) {
  ...
  const assets: Array<[string[], string]> = [
    [["SM-LAMP", "SM-LITE"], "halo-desk-lamp"], [["SM-MUG", "SM-COFF", "SM-TEA"], "cove-ceramic-mug"],
    [["SM-CHRG", "SM-KEYB", "SM-CABL", "SM-HUB", "SM-STND"], "arc-wireless-charger"], [["SM-THRW", "SM-SHTS"], "cloud-wool-throw"],
    [["SM-TOTE"], "canvas-market-tote"], [["SM-BACK"], "studio-backpack"], [["SM-WLET"], "common-leather-wallet"], [["SM-UMBR"], "rain-travel-umbrella"],
    [["SM-SPKR"], "lumen-portable-speaker"], [["SM-HEAD"], "ridge-headphones"], [["SM-POT"], "moss-plant-pot"], [["SM-CNDL"], "hearth-candle"],
    [["SM-VASE"], "north-ceramic-vase"], [["SM-BASK"], "woven-storage-basket"]
  ];
  const match = assets.find(([prefixes]) => prefixes.some((prefix) => sku.startsWith(prefix)));
  if (match) return `/catalog/${match[1]}.png`;
  const fallback = ["cove-ceramic-mug", "halo-desk-lamp", "moss-plant-pot", "studio-backpack"][Array.from(product.id).reduce((sum, char) => sum + char.charCodeAt(0), 0) % 4];
  return `/catalog/${fallback}.png`;
}
...
export default function Home() {
  ...
  const [selectedProduct, setSelectedProduct] = useState(""); const [quantity, setQuantity] = useState(1); const [activeView, setActiveView] = useState<"shop" | "orders" | "cart" | "admin">("shop"); const [nextOrdersToken, setNextOrdersToken] = useState(""); const [sidebarOpen, setSidebarOpen] = useState(false); const [searchTerm, setSearchTerm] = useState("");
